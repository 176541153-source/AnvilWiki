...
**模板现状参照**(2026-08-31 对 demo 站跑过一次全项审计,结论可作对照基线):机制层全部就绪——隐私披露/ads.txt 占位/零广告门禁/死链门禁/sitemap/法律页五件套全 Pass;**不能过审的只有内容经营本身**:demo 仅 5 篇英文文章(<15)、codes 页演示日期过期、导航挂着一个空 `items` 分类、域名是 pages.dev。这四项正好是每个 fork 用户申请前自己要补齐的事,模板机制帮不了——见 [game-selection.md](game-selection.md)(选品)与学习手册第 4 章(产页)。
