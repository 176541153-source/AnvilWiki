...
**Status (as of 2026-08-22)**: v1.19.0 released — SEO 进阶 + 2026 搜索格局对齐: learning manual ch11 `seo-traffic` (zh/en; one-page-one-keyword map, per-page 8-slot checklist, site-trust slow variables, 2026 dead-tricks list incl. FAQ rich results removal / llms.txt no-Google-effect / AI Overviews citation prefs / og:image as image-search entry; 2 new copy-paste prompts → 16→18 total, learn 10→11 chapters) + `docs/seo.md` gains a 2026 Google-official-updates log (9 entries incl. Aug 18–21 spam update & Aug 20 preferred sources, each with template impact; FAQPage note corrected to "fully removed") + `docs/content-format.md` cover-quality guidance (og:image preferred source since 2026-03) + new public `docs/roadmap.md` (8-arc evolution, near-term candidates, v2.0 direction, not-doing list); counts synced across README (zh/en) / docs/README / PRD §5 §11 §15 / landing.ts incl. announcement banner (中英). Prior: v1.18.0 — 关卡7 模板化 + 关卡8 批量内页 (scaling capabilities): `pnpm template-audit` template health check (code-layer purity via comment-stripped demo-string scan / config-layer rebrand incl. site.name / content-layer replaceability / reskin leftovers incl. content-based demo-article detection — ✅⚠️❌ scored, exits 1 only on ❌, demo repo expectedly reports ⚠️ leftovers) + `pnpm bulk-new-posts` batch scaffold (CSV/TSV keyword list → RFC-4180 parse → validate all rows (locale/category/slug conflicts/description 40–165) → write `draft: true` scaffolds only if zero hard errors — placeholder description auto-downgrades when a long title would push it past 165; existing files skipped, never overwritten; `--dry-run` preview) + `.agent/skills/anvil-batch-articles` batch content skill (intent classification → new-posts.csv → ONE unified prompt per intent → batch acceptance; anti-spam rules: no fabrication, no boilerplate reuse, real internal links only) + learning manual ch9 `templatize-your-site` & ch10 `batch-pages` (zh/en; learn 8→10 chapters, copy-paste prompts 13→16 — 13 had already drifted to 14 in v1.17.1); counts synced across README (zh/en) / docs/README / PRD §5 §11 §15 / landing.ts; pre-release expert review landed 8 fixes. Prior: v1.17.1 — expert-panel re-review of v1.17.0: demo artwork facts re-aligned with article text (meteor count/puddle duration/dodge window/whip wind-up), fork "Clear demo content" now also removes demo artwork (`clearDemoAssets` in apply-template + setup.yml sync), media prompt added to the learning handbook (zh/en). Prior: v1.17.0 — demo media enrichment: every article type now demonstrates the template's media capabilities (codes covers, boss-guide gallery mechanics diagrams, tier-list inline card images, i18n video parity, media-density guidance in docs + skills; demo art regenerable via `scripts/gen-demo-media.mjs`). Prior: v1.16.1 — expert-panel audit batch (fork-setup build break, CJK tag 404s, FAQ rich-results compliance, hreflang, ad-slot CLS, docs sync; anvilwiki-ops 0.1.3 on npm). Prior: v1.16.0 — community showcase (3 real sites, §15.6); v1.15.0 — ops toolkit `anvilwiki-ops` published to npm (see Ops Toolkit section below) + full template + AI content skills (`.agent/skills/`) + in-site docs center (`/landing/docs`: Learning manual + Development manual, zh/en, beginner-rewritten; markdown source `docs/handbook/`, kept for forks) + 9 scripts + 3 CI workflows. Live demo: anvilwiki.pages.dev (Lighthouse 4×100).
